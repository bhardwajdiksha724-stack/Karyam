...
export default function NewTimesheetForm({ employees, tasks, onCreate, onCancel, isManager, currentEmployee }) {
  const [employeeId, setEmployeeId] = useState(
    isManager ? (employees[0]?.id ?? "") : currentEmployee?.id ?? ""
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-surface border border-border rounded-lg p-5 mb-6 grid grid-cols-2 md:grid-cols-5 gap-3 items-end">
      <div>
        ...
        {isManager ? (
          ...
          <div className="w-full bg-base border border-border rounded-md px-3 py-2 text-text-muted text-sm">
      ...
      <div>
        ...
        <input type="number" step="0.5" min="0" required value={hours} onChange={(e) => setHours(e.target.value)}
          className="w-full bg-base border border-border rounded-md px-3 py-2 text-text text-sm focus:outline-none focus:ring-2 focus:ring-accent" />
      </div>
      <div className="flex gap-2">
        ...
        <button type="button" onClick={onCancel} className="text-sm text-text-muted hover:text-text px-3 py-2">
          Cancel
        </button>
      </div>
    </form>
